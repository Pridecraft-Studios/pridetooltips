...
from pridexyz.pack import generate_random_word, create_pack_metadata, compress_and_remove_directory
from pridexyz.builder import Builder
from pridexyz.tooltip.image_processing import apply_template, nine_slice_scale, make_transparent

class TooltipBuilder(Builder):

    def __init__(self, logger, src: Path, build: Path, build_user: str, meta: dict, logger_base_indent: int = 1):
    ...
    def build(self, palette: dict, palette_name: str, palette_colors: list[RGBColor]):
        ...
        for style_name in palette["tooltip"]["styles"]:
            ...
            for auto_sub_style in auto_sub_styles:
                ...
                style_description_name = style_config['description_name'] + auto_sub_style_config.get(
                    "description_name_suffix", "")
                ...
                pack_version = f"v{palette['version']}+v{style_config['version']}+v{self.meta["global_version"]}"
                pack_name = f"tooltip_{palette_name}.v{palette['version']}+{style_name}.v{style_config['version']}+v{self.meta["global_version"]}"
                ...
                try:
                    # Generate images
                    background_image = apply_template(style_config['background'], palette_colors, self.src_dir,
                                                      auto_sub_style_config.get("transpose", None))
                    frame_image = apply_template(style_config['frame'], palette_colors, self.src_dir,
                                                 auto_sub_style_config.get("transpose", None))
                    ...
                    if style_config.get('merge_background_into_frame', False):
                        ...
                        shutil.copytree(self.src_dir / "resources" / "tooltip_use_only_frame", tooltip_path, dirs_exist_ok=True)
                    else:
                    ...
                    shutil.copytree(self.src_dir / "resources" / "tooltip_common", tooltip_path, dirs_exist_ok=True)
                    ...
                    # Load overlay images
                    tooltip_text = Image.open(self.src_dir / "resources" / "pack_png_tooltip_text.png")
                    tooltip_bg = Image.open(self.src_dir / "resources" / "pack_png_tooltip_background.png")
                    pack_gallery = Image.open(self.src_dir / "resources" / "pack_gallery_background.png")
                    ...
                    pack_gallery = pack_gallery.resize((pack_gallery.width * 6, pack_gallery.height * 6),
                                                       Resampling.NEAREST)
                    ...
                    Path(build_out_path / "modrinth.md").write_text(
                        appy_modrinth_markdown_template(Path(self.src_dir / "resources" / "modrinth.md").read_text(encoding="utf-8"),
